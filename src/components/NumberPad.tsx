...
export function NumberPad({ onTap, onLongPress, onErase }: { onTap:(d:Digit)=>void; onLongPress:(d:Digit)=>void; onErase:()=>void }) {
  ...
  return <div className="pad" aria-label="Number pad">{digits.map(d=><button key={d} onPointerDown={()=>down(d)} onPointerUp={()=>up(d)} onPointerLeave={()=>timer&&clearTimeout(timer)}>{d}</button>)}<button className="erase" onClick={onErase}>Erase</button></div>;
}
